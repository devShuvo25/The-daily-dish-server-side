...
async function run(app) {
  ...
  try {
    ...
    app.patch("/update-user/:id",verifyUser, async (req, res) => {
 ...
 app.get("/meals", async (req, res) => {
  try {
    ...
    // order a meal
    app.get("/meal/:id",verifyUser, async (req, res) => {
      const id = req.params.id;
      if (id) {
        const filter = { _id: new ObjectId(id) };
        try {
    ...
    // chef's meals
    app.get("/meals/user/:email",verifyUser, async (req, res) => {
      const email = req.params.email;
      if (email) {
        // console.log(email);
    ...
    app.get("/order-request/:chefId", verifyUser, async (req, res) => {
    ...
    // chef meals delete
    app.delete("/my-meals/:id", verifyUser, async (req, res) => {
      const id = req.params.id;
      if (id) {
        const filter = { _id: new ObjectId(id) };
        try {
    ...
    app.get("/meals/:id",verifyUser, async (req, res) => {
      const id = req.params.id;
    ...
    // add to favourites
    app.post("/favourites",verifyUser, async (req, res) => {
    ...
    app.get("/favourite-meals",verifyUser, async (req, res) => {
    ...
    app.delete("/favourite-meals",verifyUser, async (req, res) => {
    ...
    // update order status
    app.post('/update-order',verifyUser,async(req,res) => {
    ...
    // my reviews
    app.get("/my-reviews",verifyUser, async (req, res) => {
    ...
    // admin apis
    app.post("/user-role",verifyUser, async (req, res) => {
      try {
    ...
    // get all request
    app.get("/user-request",verifyUser, async (req, res) => {
      res.send(
        await requestCollection.find().sort({ request_time: -1 }).toArray()
      );
    });
    // manage request
    app.patch("/user-request",verifyUser, async (req, res) => {
    ...
    app.get('/all-orders',verifyUser,async(req,res) => {
    ...
    app.get('/order-cancelled',async(req,res) => {
      const filter = {orderStatus: 'Rejected'}
      try{
        const result = await ordersCollection.find(filter).toArray()
        res.send(result)
      }
