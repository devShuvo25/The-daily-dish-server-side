  ...
  try {
    ...
    app.post("/user-role", async (req, res) => {
      ...
      if (requestData) {
        ...
        const filter = { user_email: requestData.user_email };
        ...
        if (!existingRequest) {
          try {
            ...
            console.log("Something Went wrong to Post request");
          }
        } else {
          console.log("Allready pending a request");
        }
      }
    });
    // get all request
    app.get("/user-request", async (req, res) => {
      res.send(await requestCollection.find().toArray());
    });
    // manage request
